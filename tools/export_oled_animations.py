...
EXTRA_JSON_ASSETS = {
    # Procedural firmware face. The preview JSON is generated by
    # tools/gen_status_alert.py + tools/make_face.py, but firmware renders it
    # directly in main.cpp so there is no header to parse here.
    "status_alert01": ("status_alert", "Status Alert"),
}
...
def main():
    ...
    for asset, (trigger, label) in EXTRA_JSON_ASSETS.items():
        path = os.path.join(OUT_DIR, asset + ".json")
        ...
        print(f"kept {asset}.json ({parsed['frameCount']} frames)")
